(function(document, Joomla) {
	...
	function animateLeft(current_left, new_left, duration)
	{
		...
		{
			if (duration > 0)
				wrap.classList.add('wrap_animated');
		});
	...
	function loadHome(href, tab, url, text)
	{
		...
		if (nodes.fb_loader)
			nodes.fb_loader.style.display = 'block';
		...
		ajax_request = Joomla.request({
			...
			onSuccess: (response, xhr) => {
				...
				if (nodes.fb_loader)
					nodes.fb_loader.style.display = 'none';
				...
				{
					var section_title = nodes.top_liid_home.querySelector('.NavTopUL_link').textContent.trim();
				...
				if (nodes.show_menu_btn)
					nodes.show_menu_btn.classList.add('fb-active');
				...
				if (nodes.fbTopNavigation_root)
				{
					...
					{
						if ((a.id && a.id != 'top_liid_home') || a.classList.contains('NavTopUL_topics'))
							a.remove();
					});
				}
				...
				{
					if (nodes.NavLeftUL_items)
						query('.NavLeftUL_item[data-tab="topics"]').classList.add('li_selected');
					...
					if (text !== undefined)
					{
						if (nodes.fbTopNavigation_root)
						{
							...
							'<li class="NavTopUL_item NavTopUL_topics NavTopUL_lastChild"><i class="fas fa-caret-right"></i>&nbsp;&nbsp;<a class="NavTopUL_link" href="#" onclick="return false;">' + text + '</a></li>';
	...
	function resetTopics(href, tab, url, text)
	{
		...
		if (nodes.fbTopNavigation_root)
		{
			var NavTopUL_parents = nodes.fbTopNavigation_root.querySelectorAll('li.NavTopUL_parent');

			if (NavTopUL_parents)
				top_li_count = NavTopUL_parents.length;
		}
		...
		if (top_li_count === 0)
		{
			if (nodes.fbLeftNavigation_wrap)
			{
				top_li_count = parseInt(nodes.fbLeftNavigation_wrap.style.left, 10) || '0';
	...
	{
		if (loadAllTopics == 1 || query('#' + this_liid).classList.contains('NavLeftUL_endpoint'))
		{
			// Check if there is a pending ajax request
			if (typeof ajax_request !== "undefined")
				ajax_request.abort();
	...
	function contentLoaded()
	{
		...
		var content_loaded = fbLeftNavigation_core.querySelectorAll('.ul_loading').length == 0
			&& ((fbTopNavigation_wrap) ? !fbTopNavigation_wrap.classList.contains('NavTopULloading') : true)
			&& ((fbContent_paging_button) ? !fbContent_paging_button.classList.contains('page_loading') : true);
	...
	window.addEventListener('load', function() 
	{
		...
		if (leftnav && page_view == 'topic')
		{
			...
			if (liid_topicId.classList.contains('NavLeftUL_endpoint'))
				...
				var parent_ul = liid_topicId.querySelector('ul.NavLeftUL_sublist');
	...
	document.addEventListener('DOMContentLoaded', function() 
	{
		...
		if (page_view == 'topic')
		{
			...
			if (liid_topicId.querySelector('ul.NavLeftUL_sublist'))
				liid_topicId.querySelector('ul.NavLeftUL_sublist').classList.add('NavLeftUL_expanded');
			...
			if (parent_ul_class != 'NavLeftUL_parent')
			{
				...
				var first_parent_text = liid_topicId.parentNode.parentNode.querySelector('.NavLeftUL_anchor span.topicTitle').textContent;
				var first_parent_id;
				...
				if (first_parent_id)
				{
					...
					function findParents()
					{
						...
						if ($li.parentNode.parentNode.nodeName == 'LI')
						{
							...
							var prev_parent_text = $id.querySelector('.NavLeftUL_anchor span.topicTitle').textContent;
						...
						_li.classList.add('NavTopUL_item', 'NavTopUL_topic', 'NavTopUL_parent');
						_li.innerHTML = '<i class="fas fa-caret-right"></i>&nbsp;&nbsp;<a class="NavTopUL_link" href="#" onclick="return false;">' + prev_parent_text + '</a>';
					...
					if (ul_level_num > 0)
					{
						...
						if (nodes.fbTopNavigation_root && liid_topicId.classList.contains('NavLeftUL_endpoint'))
						{
							nodes.fbTopNavigation_root.innerHTML += 
							'<li id="top_' + first_parent_id + '" class="NavTopUL_item NavTopUL_topic NavTopUL_parent NavTopUL_lastChild"><i class="fas fa-caret-right"></i>&nbsp;&nbsp;<a class="NavTopUL_link" href="#" onclick="return false;">' + first_parent_text + '</a></li>';
						}
						// Not endpoint topic - don't add class NavTopUL_lastChild
						else
						{
							nodes.fbTopNavigation_root.innerHTML += 
							'<li id="top_' + first_parent_id + '" class="NavTopUL_item NavTopUL_topic NavTopUL_parent"><i class="fas fa-caret-right"></i>&nbsp;&nbsp;<a class="NavTopUL_link" href="#" onclick="return false;">' + first_parent_text + '</a></li>';
						}
					}
				}

				// Add lastChild li in top navigation
				var last_topic_text = liid_topicId.querySelector('.NavLeftUL_anchor span.topicTitle').textContent;

				// Endpoint topic - don't add class NavTopUL_parent
				if (nodes.fbTopNavigation_root && liid_topicId.classList.contains('NavLeftUL_endpoint'))
				{
					nodes.fbTopNavigation_root.innerHTML +=
					'<li id="top_liid' + topicId + '" class="NavTopUL_item NavTopUL_topic NavTopUL_endpoint NavTopUL_lastChild"><i class="fas fa-caret-right"></i>&nbsp;&nbsp;<a class="NavTopUL_link" href="#" onclick="return false;">' + last_topic_text + '</a></li>';
				}
				// Endpoint topic - add class NavTopUL_parent
				else
				{
					nodes.fbTopNavigation_root.innerHTML +=
					'<li id="top_liid' + topicId + '" class="NavTopUL_item NavTopUL_topic NavTopUL_parent NavTopUL_lastChild"><i class="fas fa-caret-right"></i>&nbsp;&nbsp;<a class="NavTopUL_link" href="#" onclick="return false;">' + last_topic_text + '</a></li>';
				}
			}
		}
		...
		if (nodes.NavLeftUL_anchors)
		{
			nodes.NavLeftUL_anchors.forEach(function(a)
			{
				a.addEventListener('click', function(e)
				{
					...
					{
						// Topic links
						if (e.target && e.target.closest('.NavLeftUL_topic') && !e.target.closest('.NavLeftUL_backItem'))
						{
							var _this = e.target;
							...
							if (!_this.closest('li').classList.contains('NavLeftUL_endpoint'))
							{
								query('#' + this_liid).querySelector('ul').classList.add('NavLeftUL_expanded');
								...
								animation.onfinish = function()
								{
									...
									if (nodes.fbTopNavigation_root)
									{
										nodes.fbTopNavigation_root.querySelectorAll('li').forEach(function(a)
										{
										...
										// Add topic to breadcrumbs
										var this_title = query('#' + this_liid).querySelector('a').textContent;
										nodes.fbTopNavigation_root.innerHTML +=
										'<li id="top_' + this_liid + '" class="NavTopUL_item NavTopUL_topic NavTopUL_parent NavTopUL_lastChild"><i class="fas fa-caret-right"></i>&nbsp;&nbsp;<a class="NavTopUL_link" href="#" onclick="return false;">' + this_title + '</a></li>';
									}
							...
							else
							{
								...
								if (nodes.fbTopNavigation_root)
								{
									nodes.fbTopNavigation_root.querySelector('li.NavTopUL_section').classList.remove('NavTopUL_lastChild');

									// Remove last endpoint
									nodes.fbTopNavigation_root.querySelectorAll('li.NavTopUL_endpoint').forEach(function(a)
									{
										a.remove();
									});
									...
									nodes.fbTopNavigation_root.innerHTML +=
									'<li id="top_' + this_liid + '" class="NavTopUL_item NavTopUL_topic NavTopUL_endpoint NavTopUL_lastChild"><i class="fas fa-caret-right"></i>&nbsp;&nbsp;<a class="NavTopUL_link" href="#" onclick="return false;">' + this_title + '</a></li>';
						...
						if (e.target && e.target.closest('.NavLeftUL_backItem'))
						{
							...
							// Fix left navigation topics height
							var back_child_ul = _this.parentNode.parentNode.parentNode.parentNode;
							...
							{
								wrap.style.left = new_left + '%';
								query('#' + this_backliid).closest('ul').classList.remove('NavLeftUL_expanded');

								if (nodes.fbTopNavigation_root)
								{
									// Remove last endpoint
									nodes.fbTopNavigation_root.querySelectorAll('li.NavTopUL_lastChild').forEach(function(a)
									{
										a.remove();
									});
									
									nodes.fbTopNavigation_root.lastChild.classList.add('NavTopUL_lastChild');
								}
		...
		if (nodes.fbTopNavigation_root)
		{
			...
			{
				if (e.target && e.target.classList.contains('NavTopUL_link'))
				{
					e.preventDefault();
				
					var _this = e.target;

					if (loadAllTopics == 0 && _this.parentNode.classList.contains('NavTopUL_parent'))
						return;
	
					// Topic links
					if (_this.parentNode.classList.contains('NavTopUL_parent') && contentLoaded())
					{
						var NavTopUL_parents = nodes.fbTopNavigation_root.querySelectorAll('.NavTopUL_parent');
						var top_li_count = NavTopUL_parents.length;
						var li_index = [].slice.call(NavTopUL_parents).indexOf(_this.parentNode);
						var slide_count = parseInt(top_li_count, 10) - parseInt(li_index, 10) - 1;

						// Remove li after specific index
						nodes.fbTopNavigation_root.querySelectorAll('.NavTopUL_topic').forEach(function(a, i)
						{
							if (i > li_index)
								a.remove();
						});
						...
						if (!wrap.classList.contains('wrap_animated'))
						{
							...
							animation.onfinish = function()
							{
								...
								if (leftnav_li.querySelector('ul').querySelector('ul'))
									leftnav_li.querySelector('ul').querySelector('ul').classList.remove('NavLeftUL_expanded');
								...
								var topic_title = leftnav_li.querySelector('a span.topicTitle').textContent;
					...
					// Home link
					if (_this.parentNode.classList.contains('NavTopUL_section') && contentLoaded())
					{
						if (nodes.fbTopNavigation_wrap)
							nodes.fbTopNavigation_wrap.classList.add('NavTopULloading');
		...
		nodes.fbContent_core.addEventListener('click', function(e)
		{
			...
			if (e.target && (e.target.classList.contains('fbContent_topic_paging') || e.target.closest('.fbContent_topic_paging')))
			{
				e.preventDefault();

				if (e.target.classList.contains('fbContent_btn_disabled') || e.target.classList.contains('fbContent_noresults') || e.target.classList.contains('fbContent_paging_loader'))
					return false;
				...
				if (contentLoaded())
				{
					...
					_this.querySelector('.fbContent_paging_loader').style.display = 'inline-block';
					...
					ajax_request = Joomla.request({
						...
						onSuccess: (response, xhr) => {
							...
							fbPaging_topicId.querySelector('.fbContent_paging_loader').style.display = 'none';

							if (response)
							{
								fbTopic_topicId.querySelector('.fbContent_qList').innerHTML += response;
								fbPaging_topicId.querySelector('.fbContent_paging_text').style.display = 'block';
								fbPaging_topicId.querySelector('.fbContent_paging_button').classList.remove('fbContent_btn_disabled');
								var endofitems = fbTopic_topicId.querySelector('.endofitems');

								if (endofitems)
								{
									fbPaging_topicId.querySelector('.fbContent_paging_text').style.display = 'none';
									fbPaging_topicId.querySelector('.fbContent_noresults').style.display = 'block';
									fbPaging_topicId.querySelector('.fbContent_paging_button').classList.add('fbContent_btn_disabled');
								}
								else
								{
									// Increment data-page
									var new_page = page + 1;
									fbPaging_topicId.querySelector('.fbContent_paging_button').setAttribute('data-page', new_page);
								}
							}
							else
							{
							...
							fbPaging_topicId.querySelector('.fbContent_paging_button').classList.remove('page_loading');
						},
						onError: (xhr) => {
							console.log(xhr);
						}
			...
			// Toggle FAQs
			if (e.target && (e.target.classList.contains('qFaqItem_link') || e.target.closest('.qFaqItem_link')))
			{
				...
				if (_this.closest('.fbContent_qFaqItemInner').classList.contains('faq_open'))
				{
					_this.closest('.fbContent_qFaqItemInner').classList.remove('faq_open');
					_this.closest('.fbContent_qFaqItemInner').querySelector('.qFaqItem_triangle .fas').classList.replace('fa-caret-down', 'fa-caret-right');
				}
				else
				{
					_this.closest('.fbContent_qFaqItemInner').classList.add('faq_open');
					_this.closest('.fbContent_qFaqItemInner').querySelector('.qFaqItem_triangle .fas').classList.replace('fa-caret-right', 'fa-caret-down');
				}
			}
		});
		...
		if (nodes.fbLeftNavigation_core)
		{
			...
			{
				if (e.target && (e.target.classList.contains('NavLeftUL_toggle') || e.target.closest('.NavLeftUL_toggle')))
				{
					e.preventDefault();
					...
					{
						query('.fbLeftNavigation_toggle .NavLeftUL_toggle').innerHTML = '<i class="fas fa-angle-double-right"></i>';
						var minimized = 'on';
					}
					else
					{
						query('.fbLeftNavigation_toggle .NavLeftUL_toggle').innerHTML = '<i class="fas fa-angle-double-left"></i>';
						var minimized = 'off';
					}

					ajax_request = Joomla.request({
						url: site_path + "index.php?option=com_faqbookpro&task=section.toggleLeftnav&minimized=" + minimized,
						method: 'POST',
						onSuccess: (response, xhr) => {},
						onError: (xhr) => {
							console.log(xhr);
