	...
	foreach ($this->options['topic']->questions as $key => $question) {
		...
		<div class="fbContent_qListItem <?php echo $question->qListItem_class; ?>">
			...
			if ($question->publish_up > $nowDate) { ?>
				<div class="scheduled-header" title="<?php echo Text::sprintf('COM_FAQBOOKPRO_SCHEDULED_QUESTION_DESC_D', $question->publish_up); ?>">
					<?php echo Text::_('COM_FAQBOOKPRO_SCHEDULED_QUESTION'); ?>
				</div>
			<?php }
			if ($question->state == -2) { ?>
			...
			<div data-id="<?php echo $question->id; ?>" class="fbContent_qFaqItemInner <?php echo $faq_open; ?>">
				<div class="qListItem_preview">
					<h3 class="qListItem_header">
						<a href="#" data-id="<?php echo $question->id; ?>" class="qListItem_link qFaqItem_link" onclick="return false;">
							<span class="qFaqItem_triangle">
								<span class="fas <?php echo $fa_class; ?>"></span>
							</span>
							<?php if ($question->pinned) { ?>
								<i class="fas fa-thumb-tack qListItem_pinned" title="<?php echo Text::_('COM_FAQBOOKPRO_PINNED_QUESTION'); ?>"></i>
							<?php } ?>
							...
							if ($this->options['questions_params']->questions_author) {
								if ($question->created_by) { ?>
									...
									if ($question->created_by_name) { ?>
										<span>
											<?php echo Text::_('COM_FAQBOOKPRO_BY'); ?>&nbsp;
											<?php echo $question->created_by_name; ?>
										</span>
							<?php
									}
								}
