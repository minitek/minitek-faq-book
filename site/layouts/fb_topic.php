...
?>
<div class="fbTopic" id="fbTopic_<?php echo $this->topic->id; ?>" data-topic-title="<?php echo $this->topic->title; ?>">
	<?php if ($this->topic_params->show_section_questions == 'active' && $this->topic_params->topicid) {
		// Topic Title
		if ($this->topic_params->show_topic_title || $can_ask_question) { ?>
			<h2 class="fbContent_title clearfix">
				<?php if ($this->topic_params->show_topic_title) { ?>
		...
		if ($this->topic_params->show_topic_description && $this->topic->description) { ?>
			<div class="fbTopic_description"><?php echo HTMLHelper::_('content.prepare', $this->topic->description); ?></div>
		<?php }
		...
		if ($this->topic_params->show_topic_questions && isset($this->topic->questions) && $this->topic->questions) { ?>
			...
			if ($questions_found > $pagination_limit) {
				...
				if (isset($this->topic_params->issection) || $this->topic->id) {
					...
					$data_attributes = 'data-page="2" data-tab="' . $this->tab . '" data-topic="' . $this->topic->id . '"';
				}
				// Profile
				else {
					$class = 'fbContent_myquestions_paging';
					$data_attributes = 'data-assigned="' .
						$this->topic_params->isprofile[2] . '" data-page="2" data-tab="' .
						$this->tab . '" data-user="' . $this->topic_params->isprofile[1] . '"';
				} ?>
